...
function initCharts() {
    // 如果Chart.js未加载，提供降级处理方案
    if (typeof Chart === 'undefined') {
        console.log('Chart.js library not loaded, applying fallback solution.');
        applyChartFallback();
        return;
    }
...
// 降级处理函数 - 当Chart.js无法加载时显示静态内容
function applyChartFallback() {
    // 为每个图表容器提供静态内容
    const chartContainers = [
        { id: 'nmp-consumption-chart', title: 'NMP生产原料及动力消耗对比' },
        { id: 'chlorine-process-chart', title: '超高纯氯气提纯工艺纯度变化' },
        { id: 'chlorine-investment-chart', title: '超高纯氯气设备投资概算' },
        { id: 'guanidine-investment-chart', title: '硝基胍及硝酸胍投资分布比例' }
    ];
    
    chartContainers.forEach(container => {
        const ctx = document.getElementById(container.id);
        if (ctx) {
            // 创建静态内容替代图表
            ...
            fallbackDiv.style.backgroundColor = '#f9fafb';
            ...
            chartDesc.textContent = '图表数据: 请联系我们获取详细数据';
            chartDesc.style.fontSize = '14px';
            chartDesc.style.color = '#6b7280';
            
            fallbackDiv.appendChild(chartTitle);
            fallbackDiv.appendChild(chartDesc);
...
function waitForChartJS() {
    ...
    function checkChartJS() {
        ...
        if (typeof Chart !== 'undefined') {
            ...
            console.log('Chart.js load timeout, applying fallback');
            applyChartFallback();
        }
    }
